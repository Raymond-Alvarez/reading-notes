...
The primary difference between the two is whether the **order of the items matters** for the list's meaning.

*   **Use an Ordered List (`<ol>`)** when the sequence is essential, such as **recipe steps**, turn-by-turn directions, or ingredients listed by decreasing proportion.
*   **Use an Unordered List (`<ul>`)** when the items can be rearranged without changing the list's purpose.
*   **The "Test":** To decide which to use, try changing the order of the items; if the meaning changes, use `<ol>`.
...
4.  The Four Parts of the Box Model
